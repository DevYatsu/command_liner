...
def generate_sh_script():  
    ...
    if platform.system() == "Windows":
        ...
        if os.path.exists(link_path):
            return print("Name '{commands_prefix}' is already used by another process: change 'commands_prefix' and retry command 'generate-sh'")
        symbolic_link_dir = link_path
    else: 
        link_path = os.path.join("/usr/local/bin/", "{commands_prefix}")
        if os.path.exists(link_path):
            return print("Name {commands_prefix} is already used by another process")
...
# there is a security to only execute this action if magik link is created by us
...
def remove_sh_file():  
    ...
    can_remove = False
    # check if script linked to magik link has our script identifier
